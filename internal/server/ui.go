...
async function load(){var r=await fetch(A+'/hosts').then(function(r){return r.json()});items=r.hosts||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.hostname||'').toLowerCase().includes(q)||(i.ip||'').toLowerCase().includes(q)||(i.os||'').toLowerCase().includes(q)});
...
if(i.hostname)h+='<div class="item-sub">'+esc(i.hostname)+'</div>';
...
if(i.ip)h+='<span>'+esc(i.ip)+'</span>';
if(i.os)h+='<span>'+esc(i.os)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.cpu_pct)h+='<span>'+i.cpu_pct+'</span>';
if(i.mem_pct)h+='<span>'+i.mem_pct+'</span>';
if(i.disk_pct)h+='<span>'+i.disk_pct+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/hosts/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",hostname:"",ip:"",os:"",status:"",cpu_pct:0,mem_pct:0,disk_pct:0,uptime:"",last_report:"",total:"",online:"",offline:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' OUTPOST</h2>';
h+='<div class="fr"><label>Name *</label><input id="f-name" value="'+esc(i.name)+'"></div>';
h+='<div class="fr"><label>Hostname</label><input id="f-hostname" value="'+esc(i.hostname)+'"></div>';
h+='<div class="fr"><label>I P</label><input id="f-ip" value="'+esc(i.ip)+'"></div>';
h+='<div class="fr"><label>O S</label><input id="f-os" value="'+esc(i.os)+'"></div>';
h+='<div class="fr"><label>Status</label><input id="f-status" value="'+esc(i.status)+'"></div>';
h+='<div class="fr"><label>C P U Pct</label><input id="f-cpu-pct" type="number" value="'+(i.cpu_pct||0)+'"></div>';
h+='<div class="fr"><label>Mem Pct</label><input id="f-mem-pct" type="number" value="'+(i.mem_pct||0)+'"></div>';
h+='<div class="fr"><label>Disk Pct</label><input id="f-disk-pct" type="number" value="'+(i.disk_pct||0)+'"></div>';
h+='<div class="fr"><label>Uptime</label><input id="f-uptime" value="'+esc(i.uptime)+'"></div>';
h+='<div class="fr"><label>Last Report</label><input id="f-last-report" value="'+esc(i.last_report)+'"></div>';
h+='<div class="fr"><label>Total</label><input id="f-total" value="'+esc(i.total)+'"></div>';
h+='<div class="fr"><label>Online</label><input id="f-online" value="'+esc(i.online)+'"></div>';
h+='<div class="fr"><label>Offline</label><input id="f-offline" value="'+esc(i.offline)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),hostname:document.getElementById('f-hostname').value.trim(),ip:document.getElementById('f-ip').value.trim(),os:document.getElementById('f-os').value.trim(),status:document.getElementById('f-status').value.trim(),cpu_pct:parseInt(document.getElementById('f-cpu-pct').value)||0,mem_pct:parseInt(document.getElementById('f-mem-pct').value)||0,disk_pct:parseInt(document.getElementById('f-disk-pct').value)||0,uptime:document.getElementById('f-uptime').value.trim(),last_report:document.getElementById('f-last-report').value.trim(),total:document.getElementById('f-total').value.trim(),online:document.getElementById('f-online').value.trim(),offline:document.getElementById('f-offline').value.trim()};
if(editId){await fetch(A+'/hosts/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/hosts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
