...
You can view your registered hosts, but registering, editing, or removing is locked until you start a 14-day free trial.
</div>
...
var clickHandler=window._trialRequired?'showTrialNudge()':'openEdit(\\''+esc(host.id)+'\\')';
var h='<div class="card '+esc(status)+'" onclick="'+clickHandler+'">';
...
// Re-render so card onclick handlers pick up trial state
...
var t=b.textContent||'';
if(t.indexOf('Register')!==-1||t.indexOf('Add')!==-1||t.indexOf('New')!==-1){
